...
const ExpenseTable = () => {
    ...
    const getExpenseData = (expenses) => {
        ...
        const filtered = all.filter(item => {
            ...
            const matchMonth = selectedMonth ? moment(item.expenseDate).format("YYYY-MM") === selectedMonth : true;
            const matchSearch = searchExpense
                ? formattedDate.includes(searchExpense) ||
                Object.values(item).some(val =>
                    typeof val === 'string' && val.toLowerCase().includes(searchExpense.toLowerCase())
                )
                : true;
        ...
        const paginated = filtered.slice((currentPage - 1) * expenseItemsPerPage, currentPage * expenseItemsPerPage);
        setFilteredExpenseList(paginated);

        const total = filtered.reduce((sum, item) => sum + parseFloat(item.expenseAmount || 0), 0);
    ...
    const renderPageNumbers = () => {
        ...
        if (totalPages <= maxPagesToShow) {
            ...
            if (currentPage <= halfMaxPagesToShow) {
                ...
                pageNumbers.push(1, "...");
                for (let i = currentPage - halfMaxPagesToShow; i <= currentPage + halfMaxPagesToShow; i++) pageNumbers.push(i);
                pageNumbers.push("...", totalPages);
            }
        }
        return pageNumbers;
    ...
    const handleNextPage = () => { if (currentPage < numberOfPages) setCurrentPage(currentPage + 1); };
    ...
    return (
        <div className="w-full bg-white rounded-xl shadow-lg border border-gray-100 p-6 ">
            ...
            <div className="flex justify-between items-center gap-4 mb-6">
                <select
                    className="px-2 py-2 border border-gray-200 rounded-md text-sm text-gray-700"
                    value={selectedMonth}
                    onChange={(e) => setSelectedMonth(e.target.value)}
                >
                    <option value="">All Months</option>
                    {Array.from({ length: 12 }).map((_, i) => {
                        const value = moment().month(i).format("YYYY-MM");
                ...
                <div className='flex items-center gap-2'>
                    <div className="relative flex max-w-md">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6E3FF3]"
                    ...
                    <div className="flex justify-end">
                        <button
                            className="bg-[#6E3FF3] text-white px-4 py-2 rounded-md flex items-center gap-2"
                            onClick={() => document.getElementById('add-new-expense-modal').showModal()}
                        >
            ...
            <div className="mt-4 text-right text-lg font-semibold text-gray-700">
                Total Amount {selectedMonth ? `for ${moment(selectedMonth).format("MMMM YYYY")}` : "(All)"}:
            ...
            <div className="text-center">
                {filteredExpenseList.length > 0 && (
                    <div className="mt-5 flex justify-between items-center">
                        <p>
                            Showing {(currentPage - 1) * expenseItemsPerPage + 1} - {Math.min(currentPage * expenseItemsPerPage, expenseList.length)} of {expenseList.length} entries
                        </p>
                        <div className='flex items-center gap-1'>
                            <button onClick={handlePrevPage} className="py-2 px-2 bg-[#6E3FF3] text-white rounded-md">
                                <BsChevronDoubleLeft />
                            </button>
                            {renderPageNumbers().map((page, index) => (
                                <button key={index} onClick={() => typeof page === "number" && handlePageClick(page)} className={`py-1 px-3 ${currentPage === page ? "bg-yellow-600" : "bg-[#6E3FF3]"} text-white rounded-md`}>
                                    {page}
                                </button>
                            ))}
                            <button onClick={handleNextPage} className="py-2 px-2 bg-[#6E3FF3] text-white rounded-md">
