...
const EmployeeProfile = () => {
    ...
    const isDeactivated =
        String(employee?.status || '').toLowerCase() === 'de-activate';
        ...
        (async () => {
            try {
                ...
                const match = all.find(
                    (e) => String(e._id) === decodeURIComponent(id)
                );
                setEmployee(match || null);
                if (!match) setError('Employee not found');
            } catch (e) {
                setError(
                    e?.response?.data?.message || 'Failed to load employee'
                );
            } finally {
    ...
    const loadAttendance = async () => {
        ...
        try {
            ...
            toast.error(
                e?.response?.data?.message || 'Failed to load attendance'
            );
            setAttendanceRows([]);
        }
    };
    ...
    const saveDesignation = async () => {
        ...
        try {
            ...
            await axiosProtect.put(
                `/admin/employee/${encodeURIComponent(
                    String(employee._id)
                )}/designation`,
            ...
            toast.error(
                e?.response?.data?.message || 'Failed to update designation'
            );
        } finally {
            setLoading(false);
        }
    ...
    const toggleStatus = async (nextStatus) => {
        ...
        const confirm = await Swal.fire({
            title:
                nextStatus === 'De-activate'
                    ? 'De-activate this employee?'
    ...
    // aggregates
    const agg = useMemo(() => {
        const presentDays = dayStatus.filter(
            (d) => d.status === 'Present'
        ).length;
        const leaveDays = dayStatus.filter(
            (d) => d.status === 'On Leave'
        ).length;
        const absentDays = dayStatus.filter(
            (d) => d.status === 'Absent'
        ).length;
    ...
    const pfToAddThisPayroll =
        baseSalary && pfStatus?.toLowerCase() === 'active'
            ? Math.round(baseSalary * pfRate)
            : 0;
    ...
    const exportCsv = () => {
        const rows = [
            ...
            ...dayStatus.map(({ dayKey, status, att }) => [
                ...
                status,
                att?.checkInTime
                    ? moment(att.checkInTime).tz(tz).format('hh:mm A')
                    : '',
                lateToMinutes(att?.lateCheckIn) || '',
                att?.checkOutTime
                    ? moment(att.checkOutTime).tz(tz).format('hh:mm A')
                    : '',
                att?.workingHourInSeconds
                    ? msToHm(att.workingHourInSeconds)
                    : '',
        ...
        a.href = url;
        a.download = `employee_${employee?.eid || ''}_${fmtDate(
            start
        )}_${fmtDate(end)}.csv`;
        a.click();
        ...
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="rounded-2xl border bg-white p-4 shadow-sm">
                    ...
                    <div className="text-xs text-slate-500 mt-1">
                        5% of salary goes to PF when salary is given
                    </div>
                </div>
                <div className="rounded-2xl border bg-white p-4 shadow-sm">
                    <div className="flex items-center gap-2 text-slate-500 text-sm">
                    ...
                    <div className="mt-2 text-3xl font-extrabold">
                        {baseSalary != null
                            ? `৳ ${pfToAddThisPayroll.toLocaleString()}`
                            : '—'}
                    </div>
                    <div className="text-xs text-slate-500 mt-1">
                        Applies only if PF is{' '}
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                ...
                <div className="rounded-2xl border bg-white p-4 shadow-sm">
                    <div className="flex items-center gap-2 text-slate-500 text-sm">
                        <Clock3 size={16} /> Total Working
                    </div>
                    <div className="mt-2 text-2xl font-extrabold">
                        {msToHm(agg.totalWorkMs)}
                    </div>
                </div>
                <div className="rounded-2xl border bg-white p-4 shadow-sm">
                    <div className="flex items-center gap-2 text-slate-500 text-sm">
                        <Clock3 size={16} /> Total OT
                    </div>
                    <div className="mt-2 text-2xl font-extrabold">
                        {msToHm(agg.totalOtMs)}
                    </div>
                </div>
            </div>
            ...
            <div className="rounded-2xl border bg-white shadow-sm">
                        ...
                        <tbody className="divide-y">
                            {dayStatus.map(({ dayKey, status, att }) => (
                                <tr key={dayKey} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-3 py-2">
                                        {att?.totalOTInSeconds
                                            ? msToHm(att.totalOTInSeconds)
                                            : '-'}
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="px-4 py-3 text-xs text-slate-500">
                    Shift-aware statuses prevent early “Absent” before this
                    employee’s shift actually starts.
                </div>
            </div>
            ...
            <div className="rounded-2xl border bg-white p-6 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Field
                        label="Relation"
                        value={employee.emergencyContactPersonRelation}
                    />
                </div>
            </div>
