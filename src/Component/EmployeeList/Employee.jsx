        ...
        try {
            ...
            const rows = att.map((r) => ({
                ...
                date:
                    r.date ||
                    new Date(
                        r.checkInTime || r.checkOutTime || Date.now()
            ...
            setTotalWorkSeconds(
                rows.reduce(
                    (s, r) => s + Number(r.workingHourInSeconds || 0),
                    0
                )
            );
            setTotalOTSeconds(
                rows.reduce((s, r) => s + Number(r.totalOTInSeconds || 0), 0)
            );
            ...
            <div className="flex gap-4 overflow-hidden h-[calc(100vh-64px)]">
                ...
                <div className="w-3/4 border! border-gray-300!  shadow rounded-md p-4 overflow-y-auto custom-scrollbar">
                    ...
                    <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
                        ...
                        />
                        <StatCard
                            label="Leave days (selected range)"
                            value={leaveDays}
                        ...
                        <StatCard
                            ...
                            value={fmtHrs(totalWorkSeconds)}
                            accent="bg-indigo-100 text-indigo-700"
                        />
                        <StatCard
                            label="Total OT hrs"
                            value={fmtHrs(totalOTSeconds)}
                    ...
                    <section className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
                        ...
                        <div className="p-3 rounded-md border border-gray-200">
                            ...
                            <div className="text-lg font-semibold">
                                {Number(pfBalance || 0).toLocaleString(
                                    undefined,
                                    { maximumFractionDigits: 2 }
                                )}
                            </div>
                        ...
                        <div className="p-3 rounded-md border border-gray-200">
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                                View salary from Dashboard (password protected)
                            </div>
                        </div>
                    </section>
                                    ...
                                    {dailyRows.length ? (
                                        dailyRows.map((r, idx) => (
                                            ...
                                            >
                                                ...
                                                <td>
                                                    {r.checkInTime
                                                        ? toTime(r.checkInTime)
                                                        : '—'}
                                                </td>
                                                <td>
                                                    {r.checkOutTime
                                                        ? toTime(r.checkOutTime)
                                                        : '—'}
                                                </td>
                                                <td>
                                                    {r.workingDisplay ||
                                                        (r.workingHourInSeconds
                                                            ? fmtHrs(
                                                                  r.workingHourInSeconds
                                                              )
                                                            : '—')}
                    ...
                    <section className="flex justify-center">
                        ...
                        >
                            <img
                                src={
                                    employee.photo ||
                                    'https://i.ibb.co/7gY0J3C/placeholder.png'
                                }
                    ...
                    <section>
                        <div className="mt-4 space-y-0.5">
                            <h2 className="font-semibold flex items-center gap-2">
                                <MdDriveFileRenameOutline className="text-[#6E3FF3] text-xl" />
                                {employee.fullName}
                            </h2>
                            <p
                                className="text-sm flex items-center gap-2 tooltip"
                                data-tip="Designation"
                            >
                                <FaUserTie className="text-[#6E3FF3]" />{' '}
                            ...
                            <p className="text-sm">
                                <a
                                    ...
                                    data-tip={`Call to ${employee.fullName}`}
                                >
                                    <FaPhoneAlt className="text-[#6E3FF3]" />{' '}
                            ...
                            <p className="text-sm">
                                <a
                                    ...
                                    className="underline tooltip flex gap-2 items-center"
                                    data-tip={`Send mail to ${employee.fullName}`}
                                >
                                    <MdEmail className="text-[#6E3FF3]" />
                                    {employee.email}
                                </a>
                            </p>
                            <p className="text-sm flex items-center gap-2">
                                <MdBloodtype className="text-[#6E3FF3]" />{' '}
                                {employee.bloodGroup}
                            </p>
                            <p className="text-sm">
                                <a
                                ...
                                >
                                    <IoMdHelpBuoy className="text-[#6E3FF3]" />{' '}
                                    {employee.emergencyContact}
                                </a>
                            </p>
                            <p
                                className="text-sm flex items-center gap-2 tooltip"
                                data-tip="Joining date"
                            >
                                <HiMiniCalendarDateRange className="text-[#6E3FF3]" />{' '}
                                Filled-up by admin
                            </p>
                            <p
                                className="text-sm flex items-center gap-2 tooltip"
                                data-tip="Shift"
                            >
                                <SiRedhatopenshift className="text-[#6E3FF3]" />{' '}
                                {workingShift}
                            </p>
                            <p className="text-sm">
                                Status:{' '}
                                <span className="border! border-green-500! rounded-md px-1 capitalize">
                                    {employee.status}
                                </span>
                            </p>
                            <p
                                className="text-sm underline cursor-pointer hover:text-[#6E3FF3] transition-all duration-200 mt-2"
                                onClick={() =>
                                    document
            ...
            <dialog id="changeProfilePic" className="modal">
                <div className="modal-box flex flex-col items-center">
                    ...
                    <button
                        ...
                        className={`mt-5 bg-[#6E3FF3] rounded-sm px-3 py-2 text-white ${
