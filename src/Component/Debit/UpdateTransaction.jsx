...
export default function UpdateTransaction({ fetchLoans, data }) {
    ...
    const onSubmit = async (formValue) => {
        try {
            ...
            const res = await fetch(
                `${import.meta.env.VITE_BASE_URL}/loans/update-loan/${
                    data._id
                }`,
                {
                    method: 'PUT',
            ...
            if (!res.ok) {
                toast.error(result.message || 'Failed to update loan');
                return;
            }
    ...
    return (
        <div>
            <button
                ...
                onClick={() =>
                    document
                        .getElementById('update-transaction-modal')
                        .showModal()
                }
            >
